  ...
  return (
    <div className="login-screen">
      <div className="login-card">
        <div className="login-logo"><div className="icon">🥐</div><h2>Pastelería Pardilla v4.2</h2></div>
        {!showConfig ? (
...
function FicharScreen({ userProfile, employees }) {
  ...
  const handleFichar = async (type) => {
    const now = new Date();
    const time = now.toLocaleTimeString("es-ES", { hour: "2-digit", minute: "2-digit" });
    const registro = { userId: userProfile.uid, employeeName: userProfile.name, date: selectedDate, type, time, timestamp: now.toISOString() };
    try { await fb().firestore().collection("registros_horarios").add(registro); setRegistros(r => [...r, { id: Date.now(), ...registro }]); }
    catch (e) { alert("Error al guardar: " + e.message); }
  };

  const downloadCSV = (records) => {
    const header = "Fecha;Empleado;Tipo;Hora;Timestamp\n";
    const rows = records.map(r => `${r.date};${r.employeeName};${r.type};${r.time};${r.timestamp}`).join("\n");
      ...
      {!isAdmin ? (
        <>
          ...
          <div className="card">
            ...
            {dayRegistros.length === 0 ? <p style={{ color: "#999" }}>No hay registros para esta fecha</p> : dayRegistros.map(r => <div key={r.id} className="registro-card"><strong>{r.type === "entrada" ? "⬆️ Entrada" : "⬇️ Salida"}</strong> - {r.time}</div>)}
          </div>
        </>
      ) : (
        <>
                ...
                <>
                  <div style={{ overflowX: "auto", marginBottom: "16px" }}>
                    <table style={{ width: "100%", borderCollapse: "collapse", fontSize: "12px" }}>
                      <thead><tr style={{ background: "#f5f5f5", borderBottom: "2px solid #ddd" }}><th style={{ padding: "8px", textAlign: "left" }}>Fecha</th><th style={{ padding: "8px", textAlign: "left" }}>Empleado</th><th style={{ padding: "8px", textAlign: "left" }}>Tipo</th><th style={{ padding: "8px", textAlign: "left" }}>Hora</th></tr></thead>
                      <tbody>{adminRegistros.map((r, idx) => <tr key={idx} style={{ borderBottom: "1px solid #eee" }}><td style={{ padding: "8px" }}>{r.date}</td><td style={{ padding: "8px" }}>{r.employeeName}</td><td style={{ padding: "8px" }}>{r.type === "entrada" ? "⬆️ Entrada" : "⬇️ Salida"}</td><td style={{ padding: "8px" }}>{r.time}</td></tr>)}</tbody>
                    </table>
                  </div>
  ...
  return (
    <div>
      <div className="header">
        <h1><span style={{ fontSize: "28px" }}>🥐</span>Pastelería Pardilla<span className="badge" style={{ marginLeft: "12px", fontSize: "11px" }}>v4.2</span></h1>
